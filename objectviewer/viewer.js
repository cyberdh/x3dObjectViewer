...
function enableAnnotation(value){
    measureEnabled = !value;
    toolTipEnabled = !value;
    annotationEnabled =value;
...
console.log('add to scene');
    ...
    var newInline = document.getElementById('x3dFile');
...
callbackFunc(this.dataset.id,"onclick",{button:event.button});
        if (event.button == 1)
        {
            if(measureEnabled)
            {
                $('#measureDiv').append(measure.toString());
                measure.tooglePoint();                
                
            }
            
            else if(annotationEnabled==true){
            annoCount++;
            shape={translation:event.hitPnt[0]+" "+event.hitPnt[1]+" "+event.hitPnt[2], size: inlineX3d._x3domNode.getVolume().diameter*.03, id:'annotation'+annoCount, type:'sphere',emissiveColor:'.9 .1 .9'};
            annoTrans=createShape(shape);
            annoTrans.setAttribute('DEF','annotation'+annoCount);
            annoTrans.setAttribute('class', 'annotationPoint');            
            document.getElementById('MYBOX').appendChild(annoTrans);
            $('#annotationDiv').append($("<p>X:"+event.hitPnt[0]+"<br/>Y: "+event.hitPnt[1]+"<br/>Z: "+event.hitPnt[2]+"</p>"));
            $('#annotationDiv').append($("<textarea id='annoText"+annoCount+" type='text'></textarea><button type='submit'>Accept annotation</button>"));
            }
            else
            {
                $('#toolTip').hide();
...
function createShape(shape) {
    var id = shape.id;
    var translation = shape.translation;
    var size = shape.size;
    var type = shape.type;
    var color = shape.emissiveColor;
    ...
    indShape = document.createElement(type)
    if (type == 'sphere') {
        indShape.setAttribute('radius', size);
    }
    x3dShape.appendChild(appearance);
    x3dShape.appendChild(indShape);
    transform.appendChild(x3dShape);
    return transform;
...
$('#v').click(function(){
console.log('vclick');
$('scene').append("<ClipPlane id='vert' DEF='ClipPlaneVertical' plane='0 0 -1 0' containerField='children'/>");
})
